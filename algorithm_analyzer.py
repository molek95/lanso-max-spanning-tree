...
def algorithm_evaluation(file_url, result_url, k):
    ...
    if not os.path.exists('algorithm_evaluations/' + str(k)):
        os.makedirs('algorithm_evaluations/' + str(k))
    res_df.to_csv('./algorithm_evaluations/' + str(k) + '/' + str(result_url)  +'.csv', index=False)